...
import { getDependencies, getInvalidPackagesTypesInfo, packageJsonPath, sortObjectKeys, isSetupValid, getTypeDependencies, initConfig } from './util';
...
export default async function main() {
  ...
  const typeDependencies = getTypeDependencies();
  const packageTypes = await getInvalidPackagesTypesInfo(dependencies);

  const invalidTypeDependencies: string[] = [];
