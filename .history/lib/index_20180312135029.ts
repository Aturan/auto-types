...
export default async function main() {
  ...
  if (invalidTypeDependencies.length > 0) {
    const packageNames = invalidTypeDependencies.map(pkg => pkg.name).join(' ');
    execSync('npm uninstall -d ' + packageNames);
    console.log(chalk.black('Check getting some invalid type package and auto uninstall it'));
    return;
  }
